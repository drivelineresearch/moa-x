...
- **Header + overview tiles** — session id, resolved proposer/refiner/
  aggregator models, success counts, wall-clock, parallel speed-up, and
...
- **3D pipeline (Three.js)** — the four layers as a live scene: scout →
  proposers → *broadcast* refiners → configured aggregator. Every proposer
...
- **Timeline (SVG Gantt)** — one bar per agent, grouped by layer, including a
  recorded Layer 3 subprocess when present. Uses
...
- **Aggregated final plan** — `final-plan.md` rendered inline (or a
  ...
  evidence bullets no longer restart every top-level step at `1`.

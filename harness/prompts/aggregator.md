...
This prompt is for the parent Claude session (Opus 4.6) that ran
`/mixture-of-agents`. It runs AFTER the Python orchestrator has finished
Layer 1 (codex + gemini + sonnet proposers) and Layer 2 (codex + gemini
broadcast refiners). The orchestrator has written
`.moa/<session>/synthesis-input.md` containing everything you need.
...
3. Three proposer outputs (JSON, wrapped in `<proposer_output id="codex|gemini|sonnet">` tags)
4. Two refiner outputs (JSON, wrapped in `<refiner_output id="codex|gemini">` tags).
   Each refiner saw ALL three proposals during its review.
...
**Proposers**: codex (gpt-5.4 xhigh), gemini (2.5-pro), sonnet (4.6)
**Refiners**: codex, gemini (broadcast — each saw all proposals)
**Aggregator**: Opus 4.6 (this session)
...
<substantive disagreements across codex / gemini / sonnet, with your
adjudication based on evidence weight from the refiner verifications>

## Where the refiners disagreed (if applicable)
<if codex-refiner and gemini-refiner came to different conclusions, show
both and explain how you chose>

## Sources consulted
<the 10-15 most relevant of the 25+ external sources, with one-line notes
and a tag like [codex-proposer], [gemini-refiner] indicating who cited it>
